...
# pcf-usage-report-task.sh - Generate detailed Task usage report for all orgs all months in one cli call
#
...
get_usage_json () {
    ...
    for g in "${!org_guids[@]}"; do
        ...
        for i in "${!A_START_DATES[@]}"; do
            # Get data
            json_data=$(curl "https://app-usage.$opt_domain/organizations/${org_guids[$g]}/task_usages?start=${A_START_DATES[$i]}&end=${A_END_DATES[$i]}" \
...
# Get usage
TODAY=$(date +"%Y-%m-%d")
if [ "$opt_start_date" = "" ]; then
    opt_start_date="$(date +"%Y")-$(date +"%m")-01"
fi
