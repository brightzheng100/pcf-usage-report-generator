...
# pcf-usage-report-ai.sh - Generate detailed AI usage report for all or any orgs all months in one cli call
#
...
#       ./pcf-usage-report-ai.sh -D ${SYS_DOMAIN} -d ${USAGE_START_DATE}
#   2) generate AI report for system domain of "sys.pcf-gcp.abc.com", starting from "2018-01-20", as JSON output
#       ./pcf-usage-report-ai.sh -D ${SYS_DOMAIN} -d ${USAGE_START_DATE} -j
...
#       ./pcf-usage-report-ai.sh -D ${SYS_DOMAIN} -d ${USAGE_START_DATE} -o ${ORG} -f year,month,org_name,instance_count,memory_in_mb_per_instance,duration_in_seconds
#   5) generate AI report for system domain of "sys.pcf-gcp.abc.com", starting from "2018-01-20", with specified fields and csv separated output for further processing
#       ./pcf-usage-report-ai.sh -D ${SYS_DOMAIN} -d ${USAGE_START_DATE} -o ${ORG} -f year,month,org_name,instance_count,memory_in_mb_per_instance,duration_in_seconds -N -F csv
...
PROPERTIES_TO_SHOW_H=("#" year month org_guid org_name instance_count memory_in_mb_per_instance duration_in_seconds space_guid space_name app_guid app_name)
PROPERTIES_TO_SHOW=(.year .month .organization_guid .organization .instance_count .memory_in_mb_per_instance .duration_in_seconds .space_guid .space_name .app_guid .app_name)
...
get_usage_json () {
    ...
    for g in "${!org_guids[@]}"; do
        ...
        for i in "${!A_START_DATES[@]}"; do
            # Get data
            json_data=$(curl "https://app-usage.$opt_domain/organizations/${org_guids[$g]}/app_usages?start=${A_START_DATES[$i]}&end=${A_END_DATES[$i]}" \
...
json_usage_app=$(get_usage_json "$json_organizations" | jq "{app_usages:.}")

# Add extra data to json_usage_app
json_usage_app=$(echo "$json_organizations"$'\n'"$json_usage_app" | \
    jq -s 'add' | \
    jq '.organizations as $organizations |
        .app_usages[] |
        .[] |= (
                .app_usages[].organization_guid = .organization_guid |
                .app_usages[].year = (.period_start|split("-")[0]) |
                .app_usages[].month = (.period_start|split("-")[1])
            ) |
        .[].app_usages[] |= (
...
if $PRINT_JSON; then
    ...
    json_usage_app_list=$(echo "$json_usage_app" |\
        jq -r ".[] |
            $POST_FILTER
    ...
    if [[ $opt_format_output == "false" ]]; then
        ...
        (echo $P_TO_SHOW_H | tr ' ' ''$opt_seperator''; \
         echo -n "$json_usage_app_list" | sort -t ''$opt_seperator'' $SORT_OPTIONS | nl -w4 $opt_nl_string) | \
            # Cut fields
            eval $CUT_FIELDS | \
        ...
        # Print headers and app_list
        (echo $P_TO_SHOW_H | tr ' ' '\t'; \
         echo -n "$json_usage_app_list" | sort -t $'\t' $SORT_OPTIONS | nl -w4) | \
            # Cut fields
            eval $CUT_FIELDS | \
            # Format columns for nice output
