...
if $PRINT_JSON; then
    ...
    if [[ $opt_format_output == "false" ]]; then
        ...
        (echo $P_TO_SHOW_H | tr ' ' '\t'; echo -n "$json_usage_app_list" | sort -t $'\t' $SORT_OPTIONS | nl -w4) | \
            # Cut fields
            eval $CUT_FIELDS | \
